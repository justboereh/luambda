...
function response.cookie(name, value)
    if not name then return error('Cookie name required') end
    if not type(value) == 'nil' or not type(value) == 'string' then return error('Invalid value type') end
